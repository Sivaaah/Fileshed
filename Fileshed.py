...
"""
    ...
    def _has_encryption_enabled(self, user_id: str) -> bool:
        ...
        self._init_db()
        rows, _ = self._db_execute(
            "SELECT 1 FROM user_encryption WHERE user_id = ?",
            (user_id,)
        )
        return len(rows) > 0
    ...
    def _remove_user_encryption(self, user_id: str) -> None:
        ...
        self._init_db()
        self._db_execute(
            "DELETE FROM user_encryption WHERE user_id = ?",
            (user_id,)
        )
...
class Tools:
    ...
    async def shed_read(
        ...
        try:
            ...
            if self._core.user_valves:
                encryption_key = getattr(self._core.user_valves, 'encryption_key', "") or ""
            dek = None
            if encryption_key and user_id:
            ...
            return self._core._format_response(True, data=result, message=f"Read {len(text_content)} characters")

        except StorageError as e:
            return self._core._format_error(e, "shed_read")
        except Exception:
            return self._core._format_response(False, message="Unexpected error while reading file")
    ...
    async def shed_read_bytes(
        ...
        try:
            ...
            return self._core._format_response(True, data=result, message=f"Read {len(content_bytes)} bytes")

        except StorageError as e:
            return self._core._format_error(e, "shed_read_bytes")
        except Exception:
            return self._core._format_response(False, message="Unexpected error while reading binary file")
...
"""
    ...
    async def shed_encryption_setup(
        ...
        try:
            ...
            return self._core._format_response(False, message="Unexpected error during encryption setup")
    ...
    async def shed_encryption_disable(
        ...
        try:
                        ...
                        if file_path.is_file() and not file_path.name.startswith('.'):
                            try:
                                ...
                                if self._core._is_encrypted_file(content):
                                    decrypted = self._core._decrypt_file_content(dek, content)
            ...
            return self._core._format_response(False, message="Unexpected error during encryption disable")
    ...
    async def shed_encryption_status(
        ...
        try:
            ...
            if db_enabled and key_configured:
                ...
                status = "key_missing"
                status_message = "Encryption is set up but key not configured in User Valves"
            ...
            return self._core._format_response(True, data=result, message=status_message)

        except StorageError as e:
            return self._core._format_error(e, "shed_encryption_status")
        except Exception:
            return self._core._format_response(False, message="Unexpected error checking encryption status")
    ...
    async def shed_encryption_migrate(
        ...
        try:
                        ...
                        if file_path.is_file() and not file_path.name.startswith('.'):
                            try:
                                ...
                                if self._core._is_encrypted_file(content):
                                    ...
                                    encrypted = self._core._encrypt_file_content(dek, content)
                                    with open(file_path, 'wb') as f:
                                        f.write(encrypted)
                                    encrypted_count += 1
                            except Exception as e:
            ...
            return self._core._format_response(False, message="Unexpected error during encryption migration")
